...
import 'package:user/data/model/response/cart_model.dart' as cart;
import 'package:user/data/model/response/cart_model.dart';
import 'package:user/data/model/response/delivery_log_model.dart';
import 'package:user/data/model/response/order_cancellation_body.dart';
import 'package:user/data/model/response/order_details_model.dart';
import 'package:user/data/model/response/order_model.dart';
import 'package:user/data/model/response/pause_log_model.dart';
import 'package:user/data/model/response/product_model.dart';
import 'package:user/data/model/response/refund_model.dart';
import 'package:user/data/model/response/response_model.dart';
import 'package:user/data/model/response/restaurant_model.dart';
import 'package:user/data/model/response/subscription_schedule_model.dart';
import 'package:user/data/model/response/timeslote_model.dart';
import 'package:user/data/repository/order_repo.dart';
import 'package:user/helper/auth_helper.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  XFile? _pickOnlineImage;
 var distancekm = 0.0.obs;
  XFile? get pickOnlineImage => _pickOnlineImage;
  ...
  Future<void> pickImage(bool image) async {
    try {
      if (image) {
        _pickOnlineImage =
  ...
  void callTrackOrderApiFOrGuest({OrderModel? orderModel, String? orderId}) {
    if (orderModel?.orderStatus != 'delivered' &&
        orderModel?.orderStatus != 'failed' &&
        orderModel?.orderStatus != 'canceled') {
      if (kDebugMode) {
      ...
      Get.find<OrderController>().timerTrackOrderForGuest(orderId.toString());
      _timer?.cancel();
  ...
  Future<void> getRunningOrders(int offset, {bool notify = true}) async {
    ...
    if (!_runningOffsetList.contains(offset)) {
      ...
      Response response = await orderRepo.getRunningOrderList(offset, "1");
      if (response.statusCode == 200) {
        if (offset == 1) {
          _runningOrderList = [];
        }
  ...
  Future<ResponseModel?> trackOrderForGuest(
      String? orderID, OrderModel? orderModel, bool fromTracking) async {
    _trackModel = null;
    _responseModel = null;
    if (!fromTracking) {
    ...
    if (orderModel == null) {
      ...
      if (response.statusCode == 200) {
        _trackModel = OrderModel.fromJson(response.body);
        _responseModel = ResponseModel(true, response.body.toString());
        callTrackOrderApiFOrGuest(orderModel: _trackModel, orderId: orderID);
      } else {
        _responseModel = ResponseModel(false, response.statusText);
        ApiChecker.checkApi(response);
      }
  ...
  Future<ResponseModel?> timerTrackOrderForGuest(String orderID) async {
    _showCancelled = false;
  ...
  Future<void> placeOrderForGuest(
    PlaceOrderBody placeOrderBody,
    Function callback,
    double amount,
  ) async {
    ...
    if (response.statusCode == 200) {
      String? message = response.body['message'];
      String orderID = response.body['order_id'].toString();
      orderRepo.sendNotificationRequestForGuest(
          orderID, AuthHelper.getGuestId());
      callback(true, message, orderID, amount);
      if (kDebugMode) {
        print('-------- Order placed successfully $orderID ----------');
      }
    ...
    _paymentMethodIndex = Get.find<SplashController>()
        ...
        : Get.find<SplashController>().configModel!.digitalPayment!
            ...
            : Get.find<SplashController>().configModel!.customerWalletStatus ==
                    1
                ? 2
                : Get.find<SplashController>().configModel!.kpay!
                    ? 3
                    : 0;
  ...
  Future<String?> storeOrderImage(XFile? imageFile) async {
    Map<String, String> body = {};
    try {
      Response response = await orderRepo.storeImage(body, imageFile);

      if (response.statusCode == 200 || response.statusCode == 201) {
        var jsonResponse = response.body["kpay_image"];
        String imagePath = jsonResponse;
        _storeImage = imagePath;
