...
import 'package:user/data/model/response/vehicle_model.dart';
import 'package:user/data/model/response/zone_model.dart';
...
import 'package:user/view/screens/auth/delivery_man_reg_success.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  AuthController( {required this.authRepo,required this.authServiceInterface}) {
  ...
  RxBool isSignUpValidate = false.obs;
  RxBool isMM = false.obs;
  ...
  Future<void> submitBusinessPlan({required int restaurantId,required String phone}) async {
    String businessPlan;
    if (businessIndex == 0) {
      businessPlan = 'commission';
      setUpBusinessPlan(
          BusinessPlanBody(
          businessPlan: businessPlan, restaurantId: restaurantId.toString(),),phone);
    } else {
  ...
  Future<ResponseModel> setUpBusinessPlan(
      BusinessPlanBody businessPlanBody, String phone) async {
    _isLoading = true;
    update();
    ...
    if (response.statusCode == 200) {
      ...
      Future.delayed(const Duration(seconds: 1),
          () => Get.to(()=>DeliveryManRegSuccess(phone:phone??"",fromPage: "reg",)));
    } else {
  ...
  Future<ResponseModel> login(String? phone, String password,
    ...
    if (response.statusCode == 200) {
      if (Get.find<SplashController>().configModel!.customerVerification! &&
          response.body['email_verified_at'] == 0) {
      } else {
        authRepo.saveUserToken(response.body['token'],
      ...
      responseModel = ResponseModel(true,
          '${response.body['email']}${response.body['email_verified_at']}${response.body['token']}');
    } else {
      responseModel = ResponseModel(false, response.statusText);
    }
    _isLoading = false;
  ...
  Future<void> registerRestaurant(RestaurantBody restaurantBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      Get.offAll(()=>BusinessPlanScreen(restaurantId: restaurantId!,phone: restaurantBody.phone!,));
  ...
  Future<void> registerDeliveryMan(DeliveryManBody deliveryManBody) async {
    ...
    if (response.statusCode == 200) {
      ...
      Get.to(()=>DeliveryManRegSuccess(phone:deliveryManBody.phone??"",fromPage: "deli",));
