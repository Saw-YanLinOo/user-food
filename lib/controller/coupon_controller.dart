...
import 'package:user/data/model/response/coupon_model.dart';
...
class CouponController extends GetxController implements GetxService {
  ...
  Future<void> getCouponList() async {
    ...
    Response response = await couponRepo
        .getCouponList(Get.find<UserController>().userInfoModel!.id??0);
  ...
  Future<double?> applyCoupon(String coupon, double order,
      double deliveryCharge, int? restaurantID) async {
    _isLoading = true;
    _discount = 0;
    update();
    ...
    if (response.statusCode == 200) {
      ...
      if (_coupon!.couponType == 'free_delivery') {
        ...
        if (_coupon!.minPurchase != null && order >= _coupon!.minPurchase!) {
