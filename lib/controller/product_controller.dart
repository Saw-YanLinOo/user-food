...
class ProductController extends GetxController implements GetxService {
  ...
  List<Product>? get featuredProdcutList => _featuredProductList;
  List<Product>? get allDiscountList => _allDiscountList;
  ...
  Future<void> getAllProductList(bool notify, int offset) async {
    if (notify) {
      update();
    }
    ...
    if (response.statusCode == 200) {
      if (offset == 1) {
        ...
        _productModel!.totalSize =
            ProductModel.fromJson(response.body).totalSize;
        _productModel!.offset = (ProductModel.fromJson(response.body).offset);
        _productModel!.limit = ProductModel.fromJson(response.body).limit;
        _productModel!.products!.length > 200
            ? _productModel!.products!.clear()
            : _productModel!.products;
        // if (_productModel!.products!.length >
        //     int.parse(_productModel!.limit.toString())) {
        //   _productModel!.products!.removeRange(
        //       0,
        //       _productModel!.products!.length -
        //           int.parse(_productModel!.limit.toString()));
        // }
    ...
    //   List<Product> productList = data['products'].map<Product>((e) {
    //     try {
    //       var product = Product.fromJson(e);
    //       print("Product: $product");
    //       return product;
    //     } catch (i) {
    //       print("..............  \n ProductError $i Product= $e\n..........");
    //       throw Exception(i.toString());
    //     }
    //   }).toList();

    //   _allProductList.addAll(productList);
  ...
  Future<void> getFeaturedProdcutList(bool notify) async {
    if (notify) {
      update();
    }
    if (_featuredProductList.isEmpty) {
      Response response = await productRepo.getAllFeaturedList();
      if (response.statusCode == 200) {
        _featuredProductList = [];
        var data = Map<String, dynamic>.from(response.body);

        List<Product> featuredProductList = data['products'].map<Product>((e) {
          try {
            var product = Product.fromJson(e);
            //print("Product: $product");
            return product;
        ...
        _featuredProductList.addAll(featuredProductList);
  ...
  Future<void> getAllDiscountList(bool notify) async {
    if (notify) {
      update();
    }
    if (_allDiscountList.isEmpty) {
      Response response = await productRepo.getAllDiscountList();
      if (response.statusCode == 200) {
        _allDiscountList = [];
        var data = Map<String, dynamic>.from(response.body);

        List<Product> discountProductList = data['products'].map<Product>((e) {
          try {
            var product = Product.fromJson(e);
          //  print("Product: $product");
            return product;
        ...
        _allDiscountList.addAll(discountProductList);
