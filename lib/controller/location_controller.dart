...
import 'package:user/data/model/response/place_details_model.dart';
import 'package:user/data/model/response/prediction_model.dart';
...
import 'package:user/view/base/custom_loader.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  Future<void> getAddressList({bool canInsertAddress = false}) async {
    ...
    print("Address List${response.body['addresses'].length}");
    ...
    if (response.statusCode == 200) {
      ...
      print("yyyyyy ${_addressList?.length}");
      if(canInsertAddress) {
  ...
  //     {bool canInsertAddress = false}) async {
  //   Response response = await locationRepo.getAllAddress();
  ...
  Future<ResponseModel> additionalNote(String name, String note) async {
    _isLoading = true;
    update();
    Response response = await locationRepo.addAdditional(name, note);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true, response.body["message"]);
    } else {
