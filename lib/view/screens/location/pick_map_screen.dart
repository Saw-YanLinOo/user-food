...
import 'package:user/view/screens/location/widget/location_search_dialog.dart';
...
Future<Uint8List> convertAssetToUnit8List(String imagePath,
 ...
 Future<void> loadCustomMarker() async {
      Uint8List currentLocationIcon =
          await convertAssetToUnit8List(Images.currentLocationIcon, width: 100);
  
      currentIcon=BitmapDescriptor.fromBytes(currentLocationIcon);
      setState(() {});
  }
  // Future<void> loadCustomMarker(int width) async {
  //   currentIcon = await BitmapDescriptor.fromAssetImage(
  //       const ImageConfiguration(devicePixelRatio: 1), Images.currentLocationIcon,);
  //   setState(() {});
  // }

  Marker getMarker() {
    LatLng position = currentLocation != null
        ? LatLng(currentLocation!.latitude!, currentLocation!.longitude!)
        : const LatLng(0, 0);

    return Marker(
      markerId: currentLocation != null
          ? const MarkerId("currentLocation")
          : const MarkerId("picklocation"),
      position: position,
      infoWindow: const InfoWindow(
        title: 'current location',
      ),
      icon: currentIcon ?? BitmapDescriptor.defaultMarker,
    );
  }
                    ...
                    : () {
                        if (locationController.pickPosition.latitude != 0 &&
                          ...
                          if (widget.fromAddAddress) {
                            ...
                            // :
                            // Get.offAllNamed(RouteHelper.additionalInput);
                          }
                        } else {
